...
class D3ForceChart extends Component {
  ...
  componentDidMount() {
    ...
    if (ForceData === undefined || JSON.stringify(ForceData) === '{}') {
      ...
      // 内部保存原始数据
      this.setState({ newNodes: ForceData.nodes, newEdges: ForceData.edges });
    }
  ...
  forceChart(
    ...
    let { force, g, zoom, drag, longPressTImer } = this;
    let { newNodes, newEdges } = this.state;
    let that = this;
    ...
    drag = d3
      ...
      .on('start', function(d) {
        longPressTImer = setTimeout(() => {
          onLongPressDown(d);
          that.setState({ singleClick: false });
        }, 500);
        if (!d3.event.active) {
          force
      ...
      .on('end', function(d) {
        setTimeout(() => {
          that.setState({ singleClick: true });
        }, 0);
        clearTimeout(longPressTImer);
        if (!d3.event.active) {
    ...
    // svg_nodes.on('blur', (d, i) => {
    //   console.log('失去焦点', that.state.newNodes, that.state.newEdges);
    //   onBlur(that.state.newNodes, that.state.newEdges);
    // });

    /*点击节点mock数据*/
    svg_nodes.on('click', (d, i) => {
      if (that.state.singleClick) {
        onUpDateData(d, i, nodes, edges, restart);
      }
    });

    function onBlur(newNodes, newEdges) {
      nodes = newNodes;
      edges = newEdges;
      /*线*/
      svg_edges = svg
  ...
  mockUpdateDate = (d, i, nodes, edges, restart) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      obj2.name = `测试${i + 1}`;
      obj2.url =
        'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1555051176446&di=d980895b38cc694e143d01878ef5477e&imgtype=0&src=http%3A%2F%2Fp1.qhimgs4.com%2Ft010204ba62043ec126.jpg';
